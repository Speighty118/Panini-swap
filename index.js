...
const androidTestersRoutes = require('./api/android_testers');
...
const ADMIN_PATHS = ['/api/admin', '/api/invites', '/api/feedback', '/api/donations', '/api/reports', '/api/announcements', '/api/ambassador/admin', '/api/founder/admin', '/api/pl2026/admin', '/api/app-launch/admin', '/api/android-testers/admin'];
...
app.use('/api/android-testers', androidTestersRoutes);
...
// Set up a cron job hitting this URL once a day. Defaults to 50/run
...
app.all('/api/internal/run-launch-email-batch', async (req, res) => {
  ...
  try {
    const result = await appLaunchRoutes.sendNextLaunchBatch(50);
