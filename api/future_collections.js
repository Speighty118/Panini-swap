...
const COLLECTIONS = [
  ...
  { key: 'copa_2028', label: 'Copa América 2028', emoji: '🇦🇷' },
  { key: 'afcon', label: 'Africa Cup of Nations', emoji: '🌍' },
  { key: 'conference_league', label: 'UEFA Conference League', emoji: '🟢' },
  { key: 'scottish_prem', label: 'Scottish Premiership Stickers', emoji: '🏴󠁧󠁢󠁳󠁣󠁴󠁿' },
  { key: 'efl', label: 'EFL Sticker Collection', emoji: '🏟️' },
];

// GET /api/future-collections/me — get this user's votes
router.get('/me', requireAuth, async (req, res) => {
  try {
    ...
    res.json({ voted: rows.map(r => r.collection_key), collections: COLLECTIONS });
  } catch (err) {
    console.error(err);
...
// POST /api/future-collections/vote — toggle a vote
router.post('/vote', requireAuth, async (req, res) => {
  const { key, selected } = req.body;
  ...
  try {
    if (selected) {
      await pool.query(
        `INSERT INTO future_collection_votes (user_id, collection_key)
...
// GET /api/future-collections/results — admin results
router.get('/results', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT collection_key, COUNT(*) AS votes
       FROM future_collection_votes
    ...
    res.json({
      results: rows.map(r => ({
        ...COLLECTIONS.find(c => c.key === r.collection_key),
